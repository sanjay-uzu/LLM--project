...
from src.vector_db_api import get_qdrant_client, search_vectors
...
def retrieve_relevant_news(context: str) -> str:
    qdrant_client = get_qdrant_client()
    search_results = search_vectors(qdrant_client, QDRANT_COLLECTION_NAME, context, top_k=3)
    relevant_news = "\n\n".join([result.payload['summary'] for result in search_results])
    return relevant_news

def build_prompt(example: Dict) -> str:
    relevant_news = retrieve_relevant_news(example["context"])
    example['relevant news']=relevant_news
